...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (cooldowns[m.sender] && Date.now() - cooldowns[m.sender] < tiempo * 1000) {
    ...
    m.reply(`${emoji3} Ya has cometido un Crimen recientemente, espera ⏱️ *${tiempo2}* para cometer tu próximo Crimen y evitar ser atrapado.`, m, fake)
    return
  }
  ...
  switch (randomOption) {
    ...
    case 1:
      ...
      conn.reply(m.chat, `${emoji2} No fuiste cuidadoso y te atraparon mientras cometias tu cirme, se restaron *-${amountSubtracted} ${moneda} 🔥* a ${senderName}.`, m, fake)
      break
    case 2:
