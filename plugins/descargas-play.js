...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  // Enviar información de la música
  const musicInfoMessage = `
...
💿 *Álbum*: ${albumname}
⏳ *Duración*: ${duration}
🔗 *Escucha en Apple Music*: [Aquí](${url})

¡Disfruta de la música! 🎶
`;

  await conn.sendMessage(m.chat, { text: musicInfoMessage }, { quoted: m });

  // Enviar archivo de audio
  const doc = {
    audio: { url: download },
    mimetype: 'audio/mp3',  // Cambié a audio/mp3
    fileName: `${name}.mp3`,
    contextInfo: {
      externalAdReply: {
