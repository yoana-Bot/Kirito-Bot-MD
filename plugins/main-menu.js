...
import { promises as fs } from 'fs' import { join } from 'path' import fetch from 'node-fetch' import { xpRange } from '../lib/levelling.js'

const categorias = { 'anime': '🌸 ANIME', 'main': '📌 INFO', 'search': '🔍 BÚSQUEDA', 'game': '🎮 JUEGOS', 'serbot': '🤖 SUB BOTS', 'rpg': '⚔️ RPG', 'sticker': '🎭 STICKERS', 'group': '👥 GRUPOS', 'premium': '💎 PREMIUM', 'downloader': '📥 DESCARGAS', 'tools': '🛠️ HERRAMIENTAS', 'fun': '🎉 DIVERSIÓN', 'nsfw': '🔞 NSFW', 'cmd': '📂 BASE DE DATOS', 'owner': '👑 ADMIN', 'audio': '🎵 AUDIOS', 'advanced': '🚀 AVANZADO', 'rcanal': '📺 R-CANAL', 'ia': '🌟 IA', };

const emojisCategorias = { 'anime': '🎴', 'main': '📌', 'search': '🔎', 'game': '🕹️', 'serbot': '🤖', 'rpg': '⚔️', 'sticker': '🎭', 'group': '👥', 'premium': '💎', 'downloader': '📥', 'tools': '🛠️', 'fun': '🎉', 'nsfw': '🔞', 'cmd': '📂', 'owner': '👑', 'audio': '🎶', 'advanced': '🚀', 'rcanal': '📺', 'ia': '🌟', };

const generarSaludo = () => { const hora = new Date().getHours(); if (hora >= 5 && hora < 12) return '🌞 ¡Buenos días!'; if (hora >= 12 && hora < 18) return '🌤 ¡Buenas tardes!'; return '🌙 ¡Buenas noches!'; };

const formatoMenu = { antes: `╔══════════════════╗\n   Bienvenido a KIRITO-BOT\n╚══════════════════╝

✎ ${generarSaludo()}, %name.

╔═══════ೋೋ═══════☾ ║┏◆━━━━━━◆❃◆━━━━━━◆ ║┃ 🤖 Modo: %modo ║┃ 📊 Nivel: %nivel ║┃ 🏆 Experiencia: %exp / %maxexp ║┃ 👥 Usuarios registrados: %totalreg ║┗◆━━━━━━◆❃◆━━━━━━◆ ╚═══════ೋೋ═══════☾ %readmore ───────────────`, cabecera: '┏━☾➥ %categoria ««✰', cuerpo: '┃%emoji %cmd %isLimit %isPremium', pie: '┗━━«✰»━━━━«✰»━━━━«✰»━━┛', despues: '🔥 By DEYLIN 🔥', };

const more = String.fromCharCode(8206); const readMore = more.repeat(4001);

const handler = async (m, { conn, usedPrefix }) => { try { const usuario = global.db.data.users[m.sender]; if (!usuario) { return conn.reply(m.chat, '❌ No estás registrado en la base de datos.', m); }
...
} catch (error) { console.error('Error en el menú:', error); conn.reply(m.chat, '❌ Error al generar el menú.', m); } };

handler.help = ['menu', 'allmenu']; handler.tags = ['main']; handler.command = ['menu', 'allmenu', 'menú']; handler.register = true;
